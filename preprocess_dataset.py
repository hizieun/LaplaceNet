...
def resize_dataset(datadir):
    ...
    for img_path in glob('{}/**/*.*'.format(root_path), recursive=True):
        ...
        img_resize = img.resize((264, 264))

        save_path = img_path.replace(f'{datadir}_bk', f'{datadir}')
...
def move_non_labeled_imgs(datadir, num_labeled):
    img_root_path = f'./data-local/images/custom/{datadir}/train+val'
    labels_path = f'./data-local/labels/custom/{datadir}/{num_labeled}_balanced_labels'
    all_labels = []
    ...
    for img_path in glob(f'{img_root_path}/**/*.*', recursive=True):
        ...
        if labeled_img not in all_labels:
            save_img_path = img_path.replace(datadir, f'{datadir}_non_labeled')
...
if __name__ =='__main__':
    # ./data-local/images/custom/{datadir}_bk' << 이 폴더에 클래스별로 폴더분류된 원본데이터셋을 위치시켜야함
    num_labeled=40
    

    for datadir in ['D_trans', 'S_trans', 'R_trans']:
